...
object Profundus {
  ...
  implicit def particles2NewSpawn(s:Seq[Particle]): AugNewParticles = new AugNewParticles(s)

  class AugNewDamageZones(s:Seq[DamageZone]) {
    def gm = NewDamageZones(s)
    def gms = Seq(gm)
  }
  implicit def damageZones2NewSpawn(s:Seq[DamageZone]): AugNewDamageZones = new AugNewDamageZones(s)

  class AugNewMessageBox(s:GameBox) {
    def gm = NewMessageBox(s)
    def gms = Seq(gm)
  }
  implicit def damageZones2NewMessageBox(s:GameBox): AugNewMessageBox = new AugNewMessageBox(s)

  class AugNewDeformations(s:Seq[Deformation]) {
    def gm = NewDeformations(s)
    def gms = Seq(gm)
  }
  implicit def deformations2NewSpawn(s:Seq[Deformation]): AugNewDeformations = new AugNewDeformations(s)
  class AugNewEmitters(s:Seq[Emitter]) {
    def gm = NewEmitters(s)
    def gms = Seq(gm)
  }
  implicit def emitters2NewEmitters(s:Seq[Emitter]): AugNewEmitters = new AugNewEmitters(s)

  class AugNewPickups(s:Seq[Pickup]) {
    def gm = NewPickups(s)
    def gms = Seq(gm)
  }
  implicit def pickup2NewPickups(s:Seq[Pickup]): AugNewPickups = new AugNewPickups(s)

  class AugNewEntitys(s:Seq[Entity]) {
    def gm = NewEntities(s)
    def gms = Seq(gm)
  }
  implicit def creature2NewEntitys(s:Seq[Entity]): AugNewEntitys = new AugNewEntitys(s)

  class AugNewDoodads(s:Seq[Doodad]) {
    def gm = NewDoodads(s)
    def gms = Seq(gm)
  }
  implicit def doodad2NewDoodads(s:Seq[Doodad]): AugNewDoodads = new AugNewDoodads(s)

  class AugNewTransaction(s:Transaction) {
    def gm = NewTransaction(s)
    def gms = Seq(gm)
  }
  implicit def transaction2NewTransaction(s:Transaction): AugNewTransaction = new AugNewTransaction(s)

  class AugDestroyEntity(s:EntityId) {
    def gm = DestroyEntity(s)
    def gms = Seq(gm)
  }
  implicit def transaction2DestroyEntity(s:EntityId): AugDestroyEntity = new AugDestroyEntity(s)

  class AugNewClimbables(s:Seq[Climbable]) {
    def gm = NewClimbables(s)
    def gms = Seq(gm)
  }
  implicit def climbables2NewClimbables(s:Seq[Climbable]): AugNewClimbables = new AugNewClimbables(s)
