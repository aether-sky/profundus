...
object GreatWorld {
   ...
   */
  case class Update(f:(GreatWorld, Unit) => (GreatWorld, Seq[GlobalMessage]), name:Option[String]) {
    def apply: (GreatWorld, Unit) => (GreatWorld, Seq[GlobalMessage]) = f.apply _
  }

  def withName(s:String)(f:(GreatWorld, Unit) => (GreatWorld, Seq[GlobalMessage])) = Update(f, s.some)
  def stdName(s:String)(f:(GreatWorld, Unit) => GreatWorld) = {
    val ff = standard(f)
    Update(ff, s.some)
  }
  def stdNoName(f:(GreatWorld, Unit) => GreatWorld) = {
    val ff = standard(f)
    Update(ff, None)
  }
  private def updateClimbRope : Update = stdName("climbRope") { case (gw, ()) =>
    val em = gw.em
    val p = gw.p
    val curState = p.fall
  ...
  private def updateItemUse : Update = stdName("itemUse") { case (gw, ()) =>
    val pp = gw.p
    val em = gw.em
    val tc = gw.cache
    val doNothing = (em, pp)
  ...
  private def updateTool : Update = stdName("updateTool") { case (gw, ()) =>
    val pp = gw.p
    val em = gw.em
  ...
  private def collectGems : Update = stdName("collectMinerals") { case (gw, ()) =>
    val pp = gw.p
    val em = gw.em
  ...
  private def updatePlayer : Update = stdName("updatePlayer") { case (gw, ()) =>
    val tm = gw.mgr
    val pp = gw.p
    val cache = gw.cache
  ...
  private def updateCache : Update = stdName("updateTerrain") { case (gw, ()) =>
    val ppos = gw.p.pos
    val cache = gw.cache
  ...
  private def updateEs : Update = withName("entityManager") { case (gw, ()) =>
    val (ns, newEm) = gw.em.update(gw.cache)
    (gw.setEm(newEm), ns)
  }

  private def updateKzs : Update = stdName("killzones") { case (gw, ()) =>
    val newKz = gw.kz.map{_.update}.flatten
    gw.setKz(newKz)
  }

  private def updateGravity : Update = stdName("gravity") { case (gw, ()) =>
    val newEm = gw.em.doGravity(gw.cache)
    gw.setEm(newEm)
  }

  private def updateCreatures : Update = withName("entities") { case (gw, ()) =>
    val pl = gw.p
    val cache = gw.cache
  ...
  private def updateDeformations : Update = stdName("deformations") { case (gw, ()) =>
    val ds = gw.ds
    val cache = gw.cache
  ...
  private def updateParticles : Update = stdName("particles") { case (gw, ()) =>
    val pm = gw.pm
    val tc = gw.cache
  ...
  private def killEntities : Update = stdName("kills") { case (gw, ()) =>
    val pp = gw.p
    val em = gw.em
    val kz = gw.kz
  ...
  private def playerSelfQuit : Update = stdNoName { case (gw, ()) =>
    val pp = gw.p
  ...
  private def updateMusicManager : Update = stdNoName { case (gw, ()) =>
    val mm = gw.mm
    val p = gw.p
  ...
  private def addDoodadLights : Update = stdName("doodadLights") { case (gw, ()) =>
    val doods = gw.doodads.map{_.update}
    val lights = doods.map{_.getLight}.flatten
    gw.addLights(lights).setDoodads(doods)
  }

  private def filterOffscreen : Update = stdName("filterOffscreen") { case (gw, ()) =>
    val pl = gw.p
  ...
  private def doTransactions : Update = stdNoName { case (gw, ()) =>
    val pl = gw.p
    val ts = gw.ts
    val em = gw.em
    import Profundus._
...
case class GreatWorld(cols:Int, rows:Int,
  ...
  def #+(up:GreatWorld.Update) = copy(updates=updates :+ (((), up)))

  private def doUpdateWrap(t:Unit, u:Update) = {
    val (gw, ns) = u.apply(this, t)
    gw.insertSpawns(ns)
  }
  //fixme: code clones
  private def doUpdate(t:Unit, u:Update) = {
    def update() = {
      doUpdateWrap(t, u)
    }
