...
object Syllabary {
  ...
  implicit def string2VowelHelper(s:(String, Double)): VowelHelper = new VowelHelper(s)
  class ConsonantHelper(s:(String, Double)) {
    def c = Consonant(s._1, s._2)
  }
  implicit def string2ConsonantHelper(s:(String, Double)): ConsonantHelper = new ConsonantHelper(s)
